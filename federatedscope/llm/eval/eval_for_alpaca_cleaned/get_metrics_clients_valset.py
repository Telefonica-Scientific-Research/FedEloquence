...
def main():
    ...
    if testset_match is None:
        raise ValueError(
            ...
            "'_clients_valset' (optionally followed by extra suffixes), "
            "e.g. 'alpaca_cleaned_8c_clients_valset_with_DA'."
        )
    beginning_valset_name = testset_match.group(1)
    judge_module = importlib.import_module(
        f"prompt_templates.{beginning_valset_name}_llm_as_a_judge"
    )
